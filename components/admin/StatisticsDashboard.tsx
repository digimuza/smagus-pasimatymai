...
const StatisticsDashboard: React.FC = () => {
  ...
  const [audienceDistribution, setAudienceDistribution] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const todayCount = sessions.filter((s) => s.startedAt >= todayStart).length;
    ...
    <div style={{ padding: '20px 0' }}>
      ...
      >
        {[
          ...
          <div key={item.label} style={cardStyle}>
            <div style={{ fontSize: '28px', fontWeight: 'bold' }}>{item.value}</div>
            <div style={{ fontSize: '13px', color: 'var(--theme-elevation-500)' }}>
              {item.label}
            </div>
          </div>
        ))}
      ...
      >
        ...
        <div style={cardStyle}>
          ...
          {topViewed.length === 0 ? (
            <p style={{ color: 'var(--theme-elevation-400)', fontSize: '14px' }}>No data yet</p>
          ) : (
            <table style={tableStyle}>
              <thead>
                <tr>
